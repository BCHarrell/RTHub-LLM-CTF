...
"""
Used for objective `cmd-injection`. Greps a file but the command is 
vulnerable to injection. Flag is given with successful `whoami` injection.

Note: This is simulated because I wrote it for DEF CON and I don't trust nobody.
Didn't want to potentially miss an injection pattern and give actual command
execution. The grep command will actually execute but it's escaped.
